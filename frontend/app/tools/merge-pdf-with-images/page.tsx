...
import { Merge, ArrowDown, ArrowUp, Trash2, Split, Image as ImageIcon, FileText } from 'lucide-react';
...
export default function MergePdfWithImagesTool() {
    ...
    const process = async (filesData: File[], options: any) => {
        ...
        let filename = "merged_with_images.pdf";
    ...
    return (
        <>
            <ToolInterface
                title="Merge PDF with Images"
                description="Combine PDF documents and Images (JPG, PNG) into a single PDF file. Perfect for adding scanned pages or photos to documents."
                icon={Merge}
            ...
            >
                <div style={{ marginTop: 20 }}>
                    ...
                    <div className="card" style={{ padding: 10, background: '#f8f9fa' }}>
                        {files.length === 0 ? (
                            <div style={{ textAlign: 'center', padding: 20, color: '#999' }}> Upload PDF or Image files to start </div>
                        ) : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                                {blocks.map((block, index) => {
                                    ...
                                    return (
                                        <div key={block.id} className="card" style={{ padding: '10px 15px', display: 'flex', alignItems: 'center', gap: 15, borderLeft: `4px solid ${isImage ? '#f59e0b' : 'var(--primary)'}` }}>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 32, height: 32, background: isImage ? '#fff7ed' : '#eef2ff', borderRadius: 6 }}>
                                                {isImage ? <ImageIcon size={18} color="#f59e0b" /> : <FileText size={18} color="var(--primary)" />}
                                            </div>
                                            ...
                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                ...
                                                <div style={{ fontSize: '0.8rem', color: '#666' }}>{isImage ? 'Image' : `${maxPages} Pages`}</div>
                                            </div>
                                            ...
                                            {isAdvanced && isImage && (
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 8, opacity: 0.5 }}>
                                                    <span style={{ fontSize: '0.85rem' }}>Range:</span>
                                                    <input type="text" value="all" disabled style={{ width: 80, padding: '4px 8px', borderRadius: 4, border: '1px solid #eee', fontSize: '0.9rem', background: '#f5f5f5' }} />
                                                </div>
                                            )}
            ...
            <ContentSection
                title="Merge PDF and Images Together"
                features={[
                    {
                        title: "Mix Formats",
                        description: "Seamlessly combine standard PDF documents with image files (JPG, PNG, WebP) in a single operation."
                    },
                    {
                        title: "Visual Organization",
                        description: "Easily reorder your files. Images are treated as single pages that you can place anywhere in the document sequence."
                    },
                    {
                        title: "Smart Conversion",
                        description: "Images are automatically converted to high-quality PDF pages matching the document structure."
                    }
                ]}
                steps={[
                    {
                        title: "Upload Files",
                        description: "Select both PDF documents and Image files from your device."
                    },
                    {
                        title: "Order Your Sequence",
                        description: "Drag and drop or use the arrows to arrange the files exactly how you want them to appear in the final document."
                    },
                    {
                        title: "Merge",
                        description: "Click 'Merge PDF with Images' to download your combined file."
                    }
                ]}
                faq={[
                    {
                        question: "Can I put an image between two PDF pages?",
                        answer: "Yes! Currently, you can place an image between two PDF *files*. To insert inside a PDF, you would need to split that PDF first."
                    },
                    {
                    ...
                    {
                        question: "Does this quality affect my PDF?",
                        answer: "We convert images to PDF attempting to maintain their original resolution and quality."
                    }
                ]}
            />
        </>
